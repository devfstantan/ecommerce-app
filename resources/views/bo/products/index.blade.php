        ...
        <table class="table table-hover">
            <thead>
                ...
                <th colspan="2" class="text-end">
                    <a href="{{ route('bo.products.create') }}" class="btn btn-outline-primary btn-sm">
                        Nouveau
                    </a>
                </th>
            </thead>
            <tbody>
                @foreach ($products as $product)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('bo.stores.show', $product->store) }}">{{ $product->store->name }}</a>
                        </td>
                        <td>
                        ...
                        <td>
                            <a href="{{ route('bo.products.show', $product) }}" class="btn btn-outline-info btn-sm">
                                <i class="fa-solid fa-eye"></i>
                            </a>
                            <a href="{{ route('bo.products.edit', $product) }}" class="btn btn-outline-warning btn-sm">
                                <i class="fa-solid fa-pen"></i>
                            </a>
                        </td>
                        <td>
                            <form action="{{ route('bo.products.destroy', $product) }}" method="POST">
                                @csrf
                                @method('DELETE')
